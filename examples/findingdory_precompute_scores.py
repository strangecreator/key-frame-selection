...
from key_frame_selection import compute_scores_from_video
from key_frame_selection.types import PipelineConfig


DEFAULT_OUTPUT_DIR = pathlib.Path(
    "/workspace/dataspace/cogmodel/key-frame-selection/datasets/findingdory/precomputed/val"
)
...
def _save_npz_atomic(
    ...
    tmp_base = output_path.with_name(f"{output_path.name}.tmp-{os.getpid()}-{time.time_ns()}")
    try:
        if compressed:
            np.savez_compressed(tmp_base, scores=scores, hsv_hists=hsv_hists)
        else:
            np.savez(tmp_base, scores=scores, hsv_hists=hsv_hists)

        actual_tmp = tmp_base if tmp_base.suffix == ".npz" else tmp_base.with_suffix(tmp_base.suffix + ".npz")
        ...
        if tmp_base.exists():
            try:
                ...
                pass
        alt_tmp = tmp_base.with_suffix(tmp_base.suffix + ".npz")
        if alt_tmp.exists():
            try:
                alt_tmp.unlink()
            except Exception:
...
def _process_one(task: tuple[str, str, bool, bool]) -> dict[str, tp.Any]:
    video_path_str, output_dir_str, overwrite, compressed = task
    ...
    try:
        ...
        cfg = PipelineConfig()
        payload = compute_scores_from_video(video_path, cfg=cfg)
...
def _discover_videos(input_dir: pathlib.Path) -> list[pathlib.Path]:
    exts = {".mp4"}
    return sorted(
        p
...
def main() -> int:
    ...
    chunksize = int(args.chunksize) if args.chunksize is not None else _default_chunksize(len(video_paths), workers)

    output_dir.mkdir(parents=True, exist_ok=True)

    tasks = [
        (str(video_path), str(output_dir), bool(args.overwrite), bool(args.compressed))
