...
# local imports
from .patching import build_patches, filter_patch_points


def lucas_kanade_track(
    old_gray_u8: np.ndarray,
    new_gray_u8: np.ndarray,
    ...
    **lk_kwargs,
    ...
    """
    Returns:
      (tracked_indices_in_old_points, new_positions_xy_for_those_indices)
    """
    ...
    H, W = old_gray_u8.shape[:2]
    patches = build_patches(W, H, nw, nh, bool(centroidal))

    tracked_flag = np.zeros(old_points_xy.shape[0], dtype=np.uint8)
    new_positions = np.zeros_like(old_points_xy, dtype=np.float32)

    for p in patches:
        old_roi = old_gray_u8[p.y : p.y + p.h, p.x : p.x + p.w]
        new_roi = new_gray_u8[p.y : p.y + p.h, p.x : p.x + p.w]
        ...
        good = np.where(status & (err <= float(max_error)))[0]
